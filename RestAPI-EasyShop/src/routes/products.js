...
router.get('/:id', (req, res) => {
    const { id } = req.params;
    console.log(id);
    mysqlConnection.query('SELECT * FROM datosyredes.products WHERE id = ?', [id], (err, rows, fields) => {
        if (!err) {
            (async () => {
...
router.post('/', upload.array('productImage', 10), (req, res) => {
    ...
    const { propietario, titulo, categoria, subcategoria, descripcion, condicion, 
        stock, precio, precioEnvio, region, comuna } = obj;
    ...
    (async () => {
        ...
        mysqlConnection.query(query, [propietario, idProduct, nro_publicacion, titulo, categoria, subcategoria, descripcion, 
            condicion, stock, precio, precioEnvio, region, comuna], (err, rows, fields) => {
            if (!err) {
